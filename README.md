...
全ページ・APIはログイン必須です。ロールは2種類あります。

| ロール | できること |
|--------|-----------|
| **管理者 (admin)** | すべての機能。最終面接シート（`/`）、最終面接結果・面談記録の閲覧、ユーザー追加・権限変更（「管理」タブ） |
| **一般 (member)** | 採用ダッシュボード（`/dashboard`）の候補者管理・選考フロー・オンボーディング・説明会・団体連携。**個人の評価・コメント（最終面接結果、面談記録）は非表示** |

### 初回セットアップ手順

1. Supabase SQL Editor で `supabase/migrations/023_app_users.sql` を実行  
   → `app_users` テーブルが作成され、初期管理者 **三木智弘（tomohiro_miki@sportsnation.jp）** が登録されます
2. デプロイ先の `/login` を開き「初回セットアップはこちら」から、初期管理者のメールアドレスとパスワードを入力してアカウント作成  
   （認証ユーザーが1人も存在しない場合のみ実行可能）
3. ログイン後、「管理」タブから他のユーザーを追加（メール・初期パスワード・権限を指定）

環境変数 `NEXT_PUBLIC_SUPABASE_ANON_KEY` が必要です（Vercel の Environment Variables に設定済みか確認してください）。
